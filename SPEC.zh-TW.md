...
**優先順序**（使用者，2026-09-26）：以下三項先做；在下面的表格中標為**優先**。

1. 較大、較難的 eval 語料（§21.3）：目前範例語料上每個變體都是 1.0，「量出來，而不是宣稱」其實什麼都沒顯示；它也是 rerank 門檻（§21.1）與決定 HyDE／doc2query 的前提。
2. 規模（§21.1）：README 寫的約十萬 chunk 上限尚未量測（R6）。
3. T5.4 `/ask` SSE 串流（§21.2）：一個回答要 10–30 秒，期間畫面沒有任何內容；需要先寫好處理引用驗證衝突的設計稿。
...
| **規模**（≤ 5,000 份文件／100k chunks）——**優先** | 接近上限的語料（可參考 `dev/spikes/gen_synthetic_corpus.clj` 產生合成語料） | 匯入耗時、查詢延遲、記憶體；`-Xmx` 建議值 |
...
| 項目 | 說明 |
|---|---|
| **T5.4 `/ask` SSE 串流**——**優先** | 與 §10.2 引用驗證有衝突：串流出去的文字已經送出，無法事後刪除無效引用。可選做法有三：事後補一段修正、client 端延後渲染、緩衝引用。需要設計稿 |
...
| **較大、較難的 eval 語料**——**優先** | 讓變體比較與門檻校準有意義（§15.2 限制）。已決定要做（使用者，2026-09-26） |

### 21.4 發布前（GitHub）

- ~~確認 `no-commit/` 的內容從未進入 git 歷史。~~ **已完成 2026-09-26**：`.gitignore` 有 `no-commit/*`，`git log --all -- no-commit` 找不到任何碰過該路徑的 commit（共檢查 137 個 commit）。
