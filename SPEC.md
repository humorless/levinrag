...
5. **A setting that cannot be applied as written fails closed** (2026-09-25): the document is not indexed (a copy indexed earlier is removed) and appears in the report's `errors`; it never falls back to the directory's groups. This covers a `_collection.edn` that cannot be parsed, holds more than one EDN value, is not a map, has a key other than `:name` / `:read-groups`, or whose `:read-groups` is not a list of strings — every document it would govern (up to the next valid declaration below it) is affected; a file whose name looks like it but is not exactly `_collection.edn` (`_Collection.edn`, `collection.edn`, `_collections.edn`) counts as a broken one; directory names are not trimmed — and a frontmatter `read_groups` that is not a list (`read_groups: hr`), has no value on its line (including a YAML block list), is misspelt (`read_group`, `read-groups`, `Read_Groups`, `readgroups`), has a quoted key or quoted items, a full-width colon, is indented or appears more than once. The frontmatter block must start on the first line (a BOM is allowed) and end with a whole `---` or `...` line; a line that looks like `read_groups` anywhere before the first heading but outside such a block is also an error (2026-09-25 review).
6. **A document that fails to re-ingest for any other reason** (for example the embedding endpoint is down) **is removed from the index** until an ingest succeeds; its old copy would otherwise stay searchable under the old, possibly wider, groups. Each document kept out by rule 5 or removed by rule 6 is logged as one `WARN` line (`[INGEST] acl fail-closed: …` / `[INGEST] removed the stale copy of …`: path and reason, no content) besides the report's `errors`.

`bb acl:report [--users <file>] [--docs]` shows the result: per group, its number of documents and the users who hold it; per user, how many documents they can read; with `--docs`, every document's groups and readers. Readers are computed with `accessible-doc-ids`, the function retrieval filters with. It warns about groups that documents use but no user holds, documents only admins can read, users without groups, user groups no document uses, and errors in the latest ingest. Users come from `app.dtlv`, or from a users file with `--users`. Paths and group names only, no document text.
...
| `/docs/*path` | Renders the source file with commonmark (HTML escaped, URLs filtered), with cited chunks highlighted, reachable directly via `?chunk=`. If the file was modified after indexing (its hash differs), non-admins get only a notice and none of the file's text, because the indexed ACL does not cover the new text, which may carry narrower `read_groups` (2026-09-25 review); admins see the current file with a notice. Documents the user cannot see return 404 |
