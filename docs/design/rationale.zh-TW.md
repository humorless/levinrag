...
[English](rationale.md) | 繁體中文

> 本文為英文版的翻譯；內容不一致時，以英文版為準。

> 精簡版在 [README](../../README.zh-TW.md#設計理念)。
...
chunk、全文索引、向量、權限與連結圖，都是從語料推導出來的衍生資料，類似 ELT 在資料倉儲裡「載入後再轉換」。它們存在同一個嵌入式資料庫裡，而且從一份文件推導出的一切（章節、chunk、全文索引、向量、權限）都在同一個交易中寫入，所以一份文件不會出現「向量更新了、權限還沒更新」的中間狀態。原子性的單位是文件，不是整次匯入：文件逐一提交，檔案已不存在的文件在匯入結束前才刪除，文件之間的連結則在第二輪解析。匯入進行中，讀取端可能看到一部分文件已經更新、另一部分還沒；collection 的權限改了，也是這樣一份一份地套用到各文件。索引隨時可以從語料完整重建，帳號則另外存放，不受重建影響。

**為什麼嵌入式資料庫在這裡合適**：一般對嵌入式資料庫的疑慮（備份、高可用、維運成熟度）多半針對資料的唯一來源。在 LevinRAG 裡，`index.dtlv` 只是衍生資料，隨時可以整個丟掉重建：不必備份索引；換 embedding 模型、斷詞器或 schema 就重建一次；測試在暫存目錄建立全新的索引，用完即刪；同一份語料可以建出不同設定的索引並排比較。除了向量以外，重建是確定性的：章節、chunk 的切點與 id、權限與連結都只由語料與設定算出，所以改了 chunker 再重建，差異只出現在 chunker 改動之處。向量來自 embedding server，向量搜尋又是近似的（HNSW），兩次建出的索引，少數候選的排序可能不同。選擇相對小眾的資料庫，風險也因此降低，資料不會被困在裡面。

限制有三：帳號、token 與 trace（`app.dtlv`）不是衍生資料，仍然需要備份；重建的成本主要在重新計算 embedding，語料大時可能要數小時（尚未量測）；重建可以在另一個目錄進行，server 照常服務，切換到新索引目前需要重啟一次（本機實測約十幾秒），不停機的熱切換列在待辦。

## 三個不變式

1. **一致性**：一份文件的所有衍生資料，一起寫入、一起刪除。
   - 檔案刪除後，文件在下一次匯入時以一個交易離開索引；從此搜尋、問答與文件頁都碰不到它。
   - 留下來的東西在索引之外：`app.dtlv` 的 trace 保留每次的問題與回答文字，回答可能引述了該文件，目前還沒有清除它們的工具。
2. **權限**：權限是檢索資料模型的一部分，不是介面上的篩選。
   - 過濾發生在檢索層內，任何離開檢索層的候選都已經過濾。
   - 全文與向量引擎無法在排序前依文件過濾，所以候選先多撈一些再過濾。權限少的使用者召回仍可能不足；這種情況會在 trace 中標記。
...
3. **可解釋**：每個候選在每個階段的名次與分數都記入 trace，並顯示在 Debug 面板上。這相當於資料庫的執行計畫與來源追溯，可以回答「為什麼是這份文件、為什麼不是那份」。它是理解與調校 RAG 的核心工具。
   - 來源追溯一直到原文：每個 chunk 都是原始檔案中一段確切的字元範圍，送進模型的每個段落也由這些範圍組成。點引用會開啟文件頁，並標示被引用的 chunk；檔案在索引之後改過的話，文件頁會提出警告。限制是：引用指到段落（幾個 chunk），不是指到句子；而且只驗證引用編號有效，不驗證段落是否支持回答的說法。
...
目前的量測與待辦見 [SPEC.md §21](../../SPEC.zh-TW.md#21-尚未完成與下一步)。
