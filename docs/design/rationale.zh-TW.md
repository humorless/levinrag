...
**為什麼嵌入式資料庫在這裡合適**：一般對嵌入式資料庫的疑慮（備份、高可用、維運成熟度）多半針對資料的唯一來源。在 LevinRAG 裡，`index.dtlv` 只是衍生資料，隨時可以整個丟掉重建：不必備份索引；換 embedding 模型、斷詞器或 schema 就重建一次；測試在暫存目錄建立全新的索引，用完即刪；同一份語料可以建出不同設定的索引並排比較。選擇相對小眾的資料庫，風險也因此降低，資料不會被困在裡面。
