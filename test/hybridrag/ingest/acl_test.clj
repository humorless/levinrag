...
(def ^:const sample-edns
  ...
  {"" {:name "全公司" :read-groups ["all"]}
   "hr" {:name "人資" :read-groups ["hr"]}
   "hr/payroll" {:name "薪資" :read-groups ["hr" "finance"]}
   "eng" {:name "工程" :read-groups ["eng"]}
   "finance" {:name "財務" :read-groups ["finance"]} })
...
(deftest test-empty-groups-mean-admin-only
  ...
  (let [admin-only {"hr" {:name "人資" :read-groups []}}]
    (is (= []
...
(deftest test-apply-acl-overrides-deny
  ...
  (let [groups #{"hr" "intern" "contractor"}]
    (is (= #{"hr" "contractor"}
           (acl/apply-acl-overrides groups [{:type :deny, :groups ["intern"]}])))))

(deftest test-apply-acl-overrides-allow
  "Apply allow overrides: add specified groups."
  (let [groups #{"hr" "finance"}]
    (is (= #{"hr" "finance" "eng"}
           (acl/apply-acl-overrides groups [{:type :allow, :groups ["eng"]}])))))
...
(deftest test-nearest-collection-edn
  ...
  (is (= {:name "人資" :read-groups ["hr"]}
         (acl/nearest-collection-edn "hr/leave.md" sample-edns)))
  (is (= {:name "薪資" :read-groups ["hr" "finance"]}
