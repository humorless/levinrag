...
(deftest test-rerank-min-score-config
  ...
  (let [cfg (ig-extras/get-config :default)]
    (is (= -7.0 (get-in cfg [:replware.levinrag.retrieval.system/search :opts :rerank-min-score])))))
...
(deftest test-dirs-single-source
  ...
  (let [test-cfg (ig-extras/get-config :test)
    ...
    (is (= "data-test" (get-in test-cfg [:replware.levinrag.ingest.runner/runner :data-dir])))
    (is (= "data-test/index.dtlv" (get-in test-cfg [:replware.levinrag.db.index-conn/index-conn :dir])))
    (is (= "data-test/app.dtlv" (get-in test-cfg [:replware.levinrag.db.app-conn/app-conn :dir])))
    (doseq [cfg [test-cfg default-cfg]]
      (is (= (get-in cfg [:replware.levinrag.ingest.runner/runner :corpus-dir])
             (get-in cfg [:replware.levinrag.server/server :corpus-dir])
             (get-in cfg [:replware.levinrag.config/paths :corpus-dir])))
      (is (= (str (get-in cfg [:replware.levinrag.config/paths :data-dir]) "/index.dtlv")
             (get-in cfg [:replware.levinrag.db.index-conn/index-conn :dir]))))))
